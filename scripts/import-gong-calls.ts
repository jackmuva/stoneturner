// One-off importer: loads gongCall.csv into the gongCall table.
// The CSV carries a trailing `userId` column that the current schema drops.
import { db } from "@/core/db/db";
import { sql } from "drizzle-orm";
import { streamCsv } from "./csv-stream";

const CSV_PATH = "gongCall.csv";
const BATCH_SIZE = 500;

// gongCall columns (schema order), userId intentionally excluded.
const TABLE_COLUMNS = [
  "id", "callId", "url", "title", "scheduled", "started", "duration",
  "primaryUserId", "direction", "system", "scope", "media", "language",
  "workspaceId", "sdrDisposition", "clientUniqueId", "customData", "purpose",
  "meetingUrl", "isPrivate", "calendarEventId",
];
...
const flush = async () => {
  ...
  const res: any = await db.run(
    sql`INSERT INTO gongCall (${COLS_SQL}) VALUES ${valuesSql} ON CONFLICT(callId) DO NOTHING`,
  );
...
const total: any = await db.get(sql`SELECT count(*) as n FROM gongCall`);
