    ...
    "/api/sync/updates/:integration": {
      POST: withCors(async (req) => {
        if (req.params.integration) {
          ...
          Promise.resolve(runSyncPipeline(supportedIntegrations[index]!.syncPipeline, true, db, undefined, target))
    ...
    "/api/sync/:integration": {
      POST: withCors(async (req) => {
        if (req.params.integration) {
          ...
          if (index === -1) return Response.json(null, { status: 400 });
          await updateSyncPipelineStatus(target, "SYNCING", db);
          Promise.resolve(runSyncPipeline(supportedIntegrations[index]!.syncPipeline, false, db, undefined, target))
            .finally(() => updateSyncPipelineStatus(target, "IDLE", db));
          return Response.json(null, { status: 200 });
        }
        return Response.json(null, { status: 400 });
