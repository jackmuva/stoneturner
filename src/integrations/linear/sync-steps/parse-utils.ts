...
export const renderComments = (comments: StoredComment[] | null): string => {
  ...
  return "\n\n## Comments\n\n" + comments
    .map((c) => `**${c.author ?? "unknown"}** (${c.createdAt ?? "unknown"}):\n\n${c.body ?? ""}`)
    .join("\n\n---\n\n");
};
...
const generateArtifact = async <T extends Artifactable>(
  ...
  const { output: analysis } = await retry(async () => await generateText({
    ...
    prompt: `Analyze the following Linear content and extract three distinct types of information:
...
3. entities: Names of people, teams, projects, issues, tools, and concepts mentioned.
