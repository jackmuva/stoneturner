...
export const batchInsertPlaudFile = async (files: PlaudFileInsert[], db: SqliteDb): Promise<void> => {
...
export const getPlaudFiles = async (offset: number = 0, db: SqliteDb): Promise<PlaudFileSelect[]> => {
...
export const getPlaudFileByFileId = async (fileId: string, db: SqliteDb): Promise<PlaudFileSelect | undefined> => {
  const [result] = await db.select().from(plaudFile).where(eq(plaudFile.fileId, fileId));
  return result;
}

export const getLatestPlaudFile = async (db: SqliteDb): Promise<PlaudFileSelect | null> => {
...
export const getPlaudFilesWithoutTranscript = async (offset: number = 0, db: SqliteDb): Promise<PlaudFileSelect[]> => {
...
export const upsertPlaudTranscript = async (transcripts: PlaudTranscriptInsert[], db: SqliteDb): Promise<void> => {
...
export const getPlaudTranscripts = async (offset: number = 0, db: SqliteDb): Promise<PlaudTranscriptSelect[]> => {
  return await db.select()
...
export const deletePlaudData = async (db: SqliteDb): Promise<void> => {
